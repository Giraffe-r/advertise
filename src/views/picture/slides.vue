        ...
        <div style="text-align: center;background-color: white;">
            <el-row>
                <el-col style="text-align: left;padding-top: 0.5%;padding-left: 0.5%;">
                    <i class="el-icon-tickets" style="font-size: medium;">图片选择</i>
...
export default {
    ...
    methods: {
        ...
        fetchDataOnWallTable(page = 1) {
            ...
            // 异步获取远程数据（ajax）
            this.page2 = page
            pictureApi.getOnWall(this.page2, this.limit2).then(
                response => {
        ...
        //上墙一位
        onWallOne(id) {
            pictureApi.onWallOne(id).then(response => {
        ...
        //取消上墙一位
        offWallOne(id) {
            pictureApi.offWallOne(id).then(response => {
        ...
        //全部取消上墙
        offWallAll() {
            pictureApi.offWallAll().then(response => {
        ...
        //修改固定方式
        updateFixedMode(id){
            pictureApi.updateFixedMode(id).then(response => {
                this.fetchDataOnWallTable(this.page)
                if (response.success) {
                    this.$message({
                        type: 'success',
