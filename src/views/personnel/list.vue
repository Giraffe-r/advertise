<template>
    <div class="app-container">
        <el-row>
            ...
            <el-col :span="22">
                <div style="text-align: center;">
                    <el-form :inline="true" class="demo-form-inline">
                        ...
                        <el-form-item>
                            <el-select v-model="searchObj.research" placeholder="部门">
            ...
            <el-col :span="2">
                <div style="text-align: right;">
                    <el-button type="success" @click="addPersonnel()" v-if="hasPerm('personnel.add')">添加人员</el-button>
                </div>
            </el-col>
        ...
        <el-table v-loading="listLoading" :data="list" stripe style="width: 100%">
            ...
            <el-table-column prop="research" label="部门" />
            ...
            <el-table-column label="操作" width="300" align="center">
                <template slot-scope="scope">
                    ...
                    <el-button type="primary" size="mini" @click="editPersonnel(scope.row.id)">编辑</el-button>
        ...
        <el-dialog title="修改简介" :visible.sync="centerDialogVisible" width="30%" center>
            <span>
                <el-input type="textarea" :rows="10" placeholder="请输入简介" v-model="personnelForUpdate.description">
                </el-input>
            </span>
            <span slot="footer" class="dialog-footer">
                <el-button @click="centerDialogVisible = false">取 消</el-button>
                <el-button type="primary" @click="updatePersonnel()">确 定</el-button>
            </span>
        </el-dialog>
...
export default {
    ...
    methods: {
        ...
        //添加人员
        addPersonnel() {
            this.$router.push({ path: '/personnel/personnel/add' })
        },

        //修改人员
        editPersonnel(id) {
            this.$router.push({ path: `/personnel/personnel/update/${id}` })
        },
        ...
        showUpdateDialog(id) {
            ...
            personnelApi.getById(id).then(response => {
                this.personnelForUpdate = response.data.personnel;
            })
        },

        //修改简介
        updatePersonnel() {
            personnelApi.updateById(this.personnelForUpdate).then((response) => {
        ...
        fetchData(page = 1) {
            ...
            personnelApi.getPageList(this.page, this.limit, this.searchObj).then(
                response => {
        ...
        removeDataById(id) {
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                // 点击确定，远程调用ajax
                return personnelApi.removeById(id)
            }).then((response) => {
                this.fetchData(this.page)
                if (response.success) {
                    this.$message({
        ...
        onWall() {
            if (this.multipleSelection.length > 0) {
                personnelApi.onWall(this.multipleSelection).then(response => {
                    this.fetchData(this.page)
                    if (response.success) {
                        this.$message({
                            type: 'success',
        ...
        offWall() {
            if (this.multipleSelection.length > 0) {
                personnelApi.offWall(this.multipleSelection).then(response => {
        ...
        removeRows() {
            ...
            this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
                ...
                // 调用api
                return personnelApi.removeRows(idList)
            }).then((response) => {
                this.fetchData(this.page)
                if (response.success) {
                    this.$message({
