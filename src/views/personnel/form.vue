<template>
  <div class="app-container">
    <el-form ref="personnel" :model="personnel" :rules="validateRules" label-width="120px">
      <el-row :gutter="20">
        <el-col :span="6">

          <el-form-item label="姓名" prop="name">
            <el-input v-model="personnel.name" />
          </el-form-item>
        </el-col>
        <el-col :span="6">
          <el-form-item label="学部" prop="research">
            <el-select v-model="personnel.research" placeholder="学部">
      ...
      <el-form-item label="请输入简介" prop="description">
        <el-input type="textarea" :rows="15" placeholder="请输入简介" v-model="personnel.description" style="width: 100%;">
        </el-input>
      </el-form-item>
      ...
      <el-row :gutter="20">
        <el-col :span="6">
          <el-form-item label="上传图片" prop="imgSrc">
            <el-upload class="avatar-uploader" :action="BASE_API+'/template/personnel/uploadImg'"
              :show-file-list="false" :on-success="handleAvatarSuccess" :before-upload="beforeAvatarUpload">
              <img v-if="personnel.imgSrc" :src="BASE_API+'/img/' + personnel.imgSrc + '.jpg'"
                class="avatar">
        ...
        <el-col :span="6">
          <el-form-item label="是否上墙" prop="onWall">
            <el-select v-model="personnel.onWall" placeholder="请选择">
...
export default {
  ...
  methods: {
    ...
    // 新增
    saveData() {
      console.log(this.academipersonnelcian)
      personnelApi.save(this.personnel).then(response => {
        // debugger
        if (response.success) {
          this.$message({
            type: 'success',
    ...
    updateData() {
      // teacher数据的获取
      personnelApi.updateById(this.personnel).then(response => {
        if (response.success) {
          this.$message({
            type: 'success',
            message: response.message
