        ...
        <div style="text-align: center;background-color: white;">
            ...
            <el-table v-loading="listLoading" :data="list" style="width: 100%" @selection-change="handleSelectionChange">
                ...
                <el-table-column label="状态" width="70" align="center">
                    <template slot-scope="scope">
                        {{ scope.row.onWall === 1 || scope.row.onWall === 2 ? "已上墙" : "未上墙" }}
                    </template>
                </el-table-column>
                ...
                <el-table-column label="操作" width="300" align="center">
                    <template slot-scope="scope">
                        <el-button type="success" size="mini" @click="onWallOne(scope.row.id)">上墙</el-button>
...
export default {
    ...
    methods: {
        ...
        fetchDataOnWallTable(page = 1) {
            ...
            // 异步获取远程数据（ajax）
            this.page2 = page
            academician.getOnWall(this.page2, this.limit2).then(
                response => {
        ...
        //上墙一位
        onWallOne(id) {
            academician.onWallOne(id).then(response => {
        ...
        //取消上墙一位
        offWallOne(id) {
            academician.offWallOne(id).then(response => {
        ...
        //全部取消上墙
        offWallAll() {
            academician.offWallAll().then(response => {
        ...
        //修改固定方式
        updateFixedMode(id){
            academician.updateFixedMode(id).then(response => {
                this.fetchDataOnWallTable(this.page)
                if (response.success) {
                    this.$message({
                        type: 'success',
